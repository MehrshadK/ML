...
for i in ls:
    split_ratio = i
    x_train, x_test, y_train, y_test = train_test_split(x, y, random_state=22, test_size=1 - split_ratio)
    print('Train set: ', len(x_train))
    print('Test set: ', len(x_test))

    x_train_labeled = np.column_stack((x_train, y_train))

    x_train_0, x_train_1, x_train_2 = [], [], []
    ...
    cov_data_2 = covariance(x_train_2)
    ...
    def g_i_x():
        ...
        for i in range(len(x_test)):
            ...
            g1x.append(g_1_x)
            g_2_x = calculate_prob(x_test[i, :], mean_data_2, cov_data_2)
            g2x.append(g_2_x)
        return [g0x, g1x, g2x]


    """ assign gi(x) to g1(x), g2(x), g3(x) by call g_i_x() function """
    gi_x_0, gi_x_1, gi_x_2 = g_i_x()

    """ predict label of train dataset """


    def predict_label(g0, g1, g2):
        predictions = []
    ...
    test_predict = predict_label(gi_x_0, gi_x_1, gi_x_2)

...
import copy
from data_generator_2 import DataGenerator_2

data = DataGenerator_2()
...
train_labels = np.zeros((len(y_train), 3))
test_labels = np.zeros((len(y_test), 3))
...
class neural_network:
    ...
    def mean_squared_error(self):
        return sum(np.power((self.y - self.output), 2)) / 3
...
def main():
    total = 0
    mse = []
    nn = neural_network(x_train.shape[1], 10, 3, learning_rate)
    while True:
